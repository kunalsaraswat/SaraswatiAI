...
export default function App() {
  ...
  // ── Mic — NO repeated permission popups ────────────────────
  function toggleMic() {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) { alert("Use Chrome or Edge for voice input."); return; }
    if (micPerm === "denied") { alert("Microphone is blocked. Enable it in browser settings → Site Settings → Microphone."); return; }
    ...
    r.onerror = err => {
      if (err.error === "not-allowed") { setMicPerm("denied"); alert("Allow microphone in browser settings to use voice input."); }
      setMicActive(false);
    };
    r.onend = () => { setMicPerm("granted"); setMicActive(false); };
    micRef.current = r;
        ...
        <>
          ...
          <div className="sidebar">
            ...
            <div className="sb-nav">
              ...
              <div className={"sb-item" + (page === "projects" ? " active" : "")} onClick={() => { setPage("history"); setShowSb(false); }}>
                <Ico.Project /><span>Projects</span>
              </div>
              <div className={"sb-item" + (page === "apps" ? " active" : "")} onClick={() => { setShowSb(false); }}>
                <Ico.Apps /><span>Apps</span>
              </div>
      ...
      {showUpgrade && (
        <div className="mbg" onClick={() => setShowUpgrade(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p>₹99/month — Unlimited everything</p>
            <div className="pbox">
            ...
            {!payDone
              ...
              : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <button className="btn btn-p" onClick={async () => {
                  ...
                  alert("✅ Request submitted! You'll be activated within 24 hours.");
        ...
        <div className="mbg" onClick={() => setAChat(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            {aChatLoad ? <div className="ld">Loading...</div> : (
              <div className="achat">
                {aChat.msgs.length === 0
                  ? <div style={{ textAlign: "center", color: "#6b7280", fontSize: 13, padding: 16 }}>No messages found</div>
                  : aChat.msgs.map((m, i) => (
