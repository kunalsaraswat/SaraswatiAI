...
.srow { display: flex; align-items: center; gap: 13px; padding: 14px 16px; border-bottom: 1px solid ${v.bd}; cursor: pointer; } .srow:last-child { border-bottom: none; }
.sicon { font-size: 20px; width: 26px; text-align: center; } .stxt { flex: 1; }
...
export default function App() {
  ...
  async function viewUserChat(u) {
    ...
    try {
      ...
      try {
        ...
        const s2 = await getDocs(q2);
        m2 = s2.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (a.createdAt?.seconds || 0) - (b.createdAt?.seconds || 0)).slice(-30);
      }
  ...
  function newChat() { setSid(Date.now().toString()); setMsgs([]); setPage("chat"); setShowMenu(false); setImgB64(null); setImgPrev(null); endVoice(); setReactions({}); setSessionTone(null); }
  ...
  if (!user) return (
    <div className="app">
      <style>{buildCSS(theme, fontSize)}</style>
    ...
    <div className="app" onClick={() => { showMenu && setShowMenu(false); showRx && setShowRx(null); }}>
      ...
      {showMenu && (
        <div className="dd" onClick={e => e.stopPropagation()}>
          <div className="ddu">
            {pPhotoUrl
              ? <img src={pPhotoUrl} alt="" style={{ width: 38, height: 38, borderRadius: "50%", objectFit: "cover", border: "2px solid #f97316", marginBottom: 5 }} />
        ...
        <div className="page">
          ...
          <div className="scard">
            <div className="srow">
              ...
              <div className="theme-grid" style={{ gap: 6 }}>
                {[{ k: "dark", bg: "#0f0f0f", label: "🌙" }, { k: "light", bg: "#f5f5f5", label: "☀️" }, { k: "blue", bg: "#0a0f1e", label: "💙" }].map(t => (
                  <button key={t.k} onClick={async () => { setTheme(t.k); await saveSetting("theme", t.k); }} style={{ background: t.bg, border: "2px solid " + (theme === t.k ? "#f97316" : "#333"), borderRadius: 10, cursor: "pointer", padding: "6px 10px", fontSize: 14 }}>{t.label}</button>
                ))}
              </div>
            </div>
            <div className="srow">
              ...
              <div style={{ display: "flex", gap: 5 }}>
                {[12, 14, 16].map(s => (
                  <button key={s} onClick={async () => { setFontSize(s); await saveSetting("fontSize", s); }} style={{ background: fontSize === s ? "#f9731618" : "transparent", border: "1px solid " + (fontSize === s ? "#f97316" : "#444"), borderRadius: 8, color: fontSize === s ? "#f97316" : "#888", cursor: "pointer", padding: "4px 9px", fontFamily: "Inter,sans-serif", fontSize: 12 }}>{s}</button>
                ))}
              </div>
            </div>
            <div className="srow">
          ...
          <div className="scard">
            <div className="srow">
              ...
              <div style={{ fontSize: 12, color: "#f97316", fontWeight: 600 }}>{sessionTone === "female" ? "👩 Female" : sessionTone === "male" ? "👨 Male" : "Auto"}</div>
            </div>
          </div>
          {/* CHAT */}
          <div className="sec">Chat</div>
          <div className="scard">
      ...
      {page === "admin" && isAdmin && (
        <div className="page">
          <div style={{ background: "#f9731614", border: "1px solid #f97316", borderRadius: 12, padding: "11px 14px", fontSize: 13, color: "#fb923c" }}>🛡️ Admin Panel</div>
          <div className="sgrid">
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 64, marginTop: 4 }}>
              {adminGraph.map((d, i) => (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ fontSize: 8, color: "#f97316", marginBottom: 2 }}>{d.v > 0 ? d.v : ""}</div>
                  <div style={{ width: "100%", background: "#f97316", borderRadius: "3px 3px 0 0", height: Math.max(d.v === 0 ? 2 : (d.v / maxG) * 50, 2), opacity: d.v === 0 ? .25 : .9 }} />
                  <div style={{ fontSize: 9, color: "#6b7280", marginTop: 3 }}>{d.l}</div>
                </div>
              ))}
            </div>
          </div>
          <div className="sbar"><IcoSearch /><input placeholder="Search users..." value={aSearch} onChange={e => setASearch(e.target.value)} /></div>
            ...
            <div key={u.id} className="ucard" style={{ flexDirection: "column", alignItems: "stretch", gap: 7 }}>
              ...
              {u.email !== ADMIN_EMAIL && (
                <div style={{ display: "flex", gap: 7 }}>
                  ...
                  <button onClick={() => adminToggle(u.id, u.premium)} style={{ flex: 1, background: u.premium ? "#ef444418" : "#22c55e18", border: "1px solid " + (u.premium ? "#ef4444" : "#22c55e"), borderRadius: 8, color: u.premium ? "#ef4444" : "#22c55e", cursor: "pointer", fontSize: 12, fontWeight: 700, padding: 7 }}>{u.premium ? "Remove" : "Give Premium"}</button>
      ...
      {aChat && (
        <div className="mbg" onClick={() => setAChat(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 11 }}>{aChat.user.email} · {aChat.user.usageCount || 0} total chats</p>
